...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/vls.$USER.serv00.net/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/vls.$USER.serv00.net/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/vls.$USER.serv00.net/vless/config.json > ~/domains/vls.$USER.serv00.net/vless/config_tmp.json && mv ~/domains/vls.$USER.serv00.net/vless/config_tmp.json ~/domains/vls.$USER.serv00.net/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/vls.$USER.serv00.net/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}vls.$USER.serv00.net${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/vls.$USER.serv00.net/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@vls.$USER.serv00.net:${port}?flow=&security=none&encryption=none&type=ws&host=vls.$USER.serv00.net&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: vls.$USER.serv00.net"
            echo -e "VLESS节点信息: vless://${uuid}@vls.$USER.serv00.net:${port}?flow=&security=none&encryption=none&type=ws&host=vls.$USER.serv00.net&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless"
